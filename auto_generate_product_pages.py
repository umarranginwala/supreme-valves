#!/usr/bin/env python3
"""
Auto-generate product pages for all products in products.json
Creates missing product pages automatically
"""
...
# Set random seed for consistent prices
random.seed(42)
...
def generate_product_page(product):
    ...
    image_url = f"https://www.supremevalves.in/{image.replace(' ', '%20')}"
...
<head>
    ...
    <title>{product_name} | Supreme Valves India</title>
    <meta name="description" content="{description} - {', '.join(applications)}. Premium quality from Supreme Valves India.">
    <meta name="keywords" content="{product_name}, {short_name}, {category}, {', '.join(applications)}, Supreme Valves India">
    ...
    {{
      ...
      "offers": {{
        "@type": "Offer",
        "url": "https://www.supremevalves.in/products/{product_id}.html",
        "priceCurrency": "INR",
        "price": "{price}",
...
<body>
    ...
    <main class="container" style="padding-top: 3rem;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 3rem; margin-bottom: 4rem;">
            <div>
                <img src="../{image}" alt="{product_name}" style="width: 100%; border-radius: 12px; box-shadow: var(--shadow-lg);">
            </div>
            <div>
                <span style="display: inline-block; padding: 0.5rem 1rem; background: var(--lighter-gray); color: var(--accent-color); border-radius: 6px; font-size: 0.85rem; font-weight: 600; margin-bottom: 1rem;">{category}</span>
                ...
                <div style="background: var(--lighter-gray); padding: 1.5rem; border-radius: 12px; margin-bottom: 2rem;">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
                        {''.join([f'<span style="padding: 0.5rem 1rem; background: white; border-radius: 6px; font-size: 0.9rem;">{app}</span>' for app in applications])}
                    </div>
                </div>

                <div style="display: flex; gap: 1rem;">
                    <a href="../contact.html?product={product_name}" class="btn btn-primary">Request Quote</a>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 2rem;">
                <div style="background: var(--lighter-gray); padding: 2rem; border-radius: 12px; border-left: 4px solid var(--accent-color);">
                    <h3 style="color: var(--primary-color); margin-bottom: 1rem;">
                        <i class="fas fa-check-circle" style="color: var(--accent-color); margin-right: 0.5rem;"></i>
                        High Quality Materials
                    </h3>
                    <p>Manufactured using premium grade materials meeting international standards for durability and performance.</p>
                </div>
                <div style="background: var(--lighter-gray); padding: 2rem; border-radius: 12px; border-left: 4px solid var(--accent-color);">
                    <h3 style="color: var(--primary-color); margin-bottom: 1rem;">
                        <i class="fas fa-check-circle" style="color: var(--accent-color); margin-right: 0.5rem;"></i>
                    ...
                    <p>All products undergo rigorous testing and quality control to ensure compliance with industry standards.</p>
                </div>
                <div style="background: var(--lighter-gray); padding: 2rem; border-radius: 12px; border-left: 4px solid var(--accent-color);">
                    <h3 style="color: var(--primary-color); margin-bottom: 1rem;">
                        <i class="fas fa-check-circle" style="color: var(--accent-color); margin-right: 0.5rem;"></i>
                    ...
                    <p>Available in various sizes and pressure ratings to meet diverse application requirements.</p>
                </div>
                <div style="background: var(--lighter-gray); padding: 2rem; border-radius: 12px; border-left: 4px solid var(--accent-color);">
                    <h3 style="color: var(--primary-color); margin-bottom: 1rem;">
                        <i class="fas fa-check-circle" style="color: var(--accent-color); margin-right: 0.5rem;"></i>
                    ...
                    <p>Technical support and consultation available to help you select the right product for your needs.</p>
                </div>
            </div>
        </section>
                ...
                <form action="../contact.html" method="get" style="display: grid; gap: 1.5rem;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                        <div>
                            ...
                            <input type="tel" name="phone" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px;">
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 0.5rem; font-weight: 600;">Company</label>
                            <input type="text" name="company" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px;">
                        </div>
                    </div>
                    <div>
                        <label style="display: block; margin-bottom: 0.5rem; font-weight: 600;">Product</label>
                        <input type="text" name="product" value="{product_name}" readonly style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px; background: var(--white-color);">
                    </div>
                    <div>
                        <label style="display: block; margin-bottom: 0.5rem; font-weight: 600;">Requirements / Message *</label>
                        <textarea name="message" required rows="5" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px; resize: vertical;"></textarea>
                    </div>
    ...
    <div class="whatsapp-widget">
        <a href="https://api.whatsapp.com/send/?phone=919773278770&text=Inquiry%20about%20{product_name.replace(' ', '%20')}" target="_blank" aria-label="Chat with us on WhatsApp">
...
# Generate pages for all products
for product in products:
    product_id = product['id']
    filepath = os.path.join(products_dir, f"{product_id}.html")
    
    if os.path.exists(filepath):
        print(f"⏭️  Already exists: {product_id}.html")
        existing_count += 1
    else:
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(html)
        print(f"✅ Created: {product_id}.html")
        created_count += 1

print(f"\n🎉 Summary:")
print(f"  Created: {created_count} new pages")
print(f"  Existing: {existing_count} pages")
print(f"  Total: {len(products)} products")
print(f"\n✅ All products now have dedicated pages!")
